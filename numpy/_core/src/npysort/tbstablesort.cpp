// This file implements the TBStableSort algorithm.
// Adapted from tbtimsort.cpp (which was adapted from tbsort.c)

#include "tbstablesort.hpp"    // Own header (includes npysort_common.h, numpy_tag.h and fwd decl for npy::sort::mergesort_)
...
namespace npy_tbstablesort { // Renamed namespace

// Helper function definitions (swap, myclamp, search, insertionSort, Bin struct)
// These are implementation details of TBStablesort (copied from TBSort)
...
// Function to implement insertion sort (used for initial runs and very small arrays)
template <typename Tag>
...
// TBStablesort function implementation (adapted from TBSort)
template <typename Tag>
int TBstablesort(typename Tag::type *arr, npy_intp l, npy_intp r) { // Renamed function
    // 1. Base Case
    if (l >= r) {
        return 0; // Success
    }
    ...
    // 2. TREE Step (Identical to TBSort)
    npy_intp treeSize;
    if (numElements < 4) {
        treeSize = 1;
    } else {
    ...
    for (npy_intp i = 0; i < treeSize; i++) {
        ...
        sampleTree[i] = arr[l + rand() % numElements]; // TODO: Replace rand()
    }
    // Sort sampleTree using a stable sort if stability of sample matters,
    // but typically sampleTree sorting stability is not critical. Sticking to insertionSort.
    insertionSort<Tag>(sampleTree, treeSize);

    // 3. BIN Step (Identical to TBSort)
    npy_intp binCount;
    ...
    for (npy_intp i = 0; i < numElements; i++) {
        ...
        if (mypos == -1) {
            slope_offset_idx = 0;
        } else if (mypos == treeSize - 1 && !Tag::less(sampleTree[treeSize - 1], element_val)) {
    ...
    // 4. SORT Step - Modified for TBStablesort
    ...
    for (npy_intp i = 0; i < binCount; i++) {
        ...
        if (bins[i].size < binThreshold) {
            // Call NumPy's Mergesort for small bins (stable sort)
            if (npy::sort::mergesort_<Tag>(bins[i].elements, bins[i].size) != 0) { // Check return
                 // Error in mergesort, propagate error up
                delete[] sampleTree; if (targetbin) delete[] targetbin; if (slope_arr) delete[] slope_arr; if (offset_arr) delete[] offset_arr;
                for(npy_intp k=0; k<binCount; ++k) if (bins[k].elements) delete[] bins[k].elements;
                delete[] bins;
                return -1; // Error
            }
        } else {
            // Recursive call to TBStablesort
            if (TBStablesort<Tag>(bins[i].elements, 0, bins[i].size - 1) != 0) { // Renamed recursive call
        ...
        if (curpos + bins[i].size > r + 1) {
            // Error: TBStablesort trying to write out of bounds.
            delete[] sampleTree;
            if (targetbin) delete[] targetbin;
            if (slope_arr) delete[] slope_arr;
            if (offset_arr) delete[] offset_arr;
...
template int npy_tbstablesort::TBstablesort<npy::tag::SignedInt>(npy::tag::SignedInt::type *, npy_intp, npy_intp);
template int npy_tbstablesort::TBstablesort<npy::tag::Float>(npy::tag::Float::type *, npy_intp, npy_intp);
// template int npy_tbstablesort::TBstablesort<npy::tag::UnsignedInt>(npy::tag::UnsignedInt::type *, npy_intp, npy_intp);
// template int npy_tbstablesort::TBstablesort<npy::tag::Double>(npy::tag::Double::type *, npy_intp, npy_intp);
